    ...
    protected function _getButtonField($fieldname, $args = array()) {
        ...
        $html = '<div class="std-form-line field-' . $fieldname . (isset($options['type'])  && $options['type'] == 'hidden' ? ' hide-field' : '') . (isset($options['width']) ? ' half' : '') . ' field-button">
                    ...
                    <div class="grp ' . (isset($options['classes']) ? $options['classes'] : '') . '">
                        <input title="' . $options['label'] . '" class="std-input text" type="' . $options['type'] . '" placeholder="Label" id="' . $el_id . '-label" name="' . $name  . '[label]" value="' . (isset($value['label']) ? $value['label'] : '') . '" ' . implode(' ', $attributes) . ' />
                        
                        <input title="' . $options['label'] . '" class="std-input url" type="' . $options['type'] . '" placeholder="URL" id="' . $el_id . '-link" name="' . $name  . '[link]" value="' . (isset($value['link']) ? $value['link'] : '') . '" ' . implode(' ', $attributes) . ' />
                        
                        <select name="' . $name  . '[target]" id="' . $el_id . '-target" class="type">
                            <option value="_self" ' . (isset($value['target']) && $value['target'] == '_self' ? 'selected="selected"' : '') . '>_self</option>
                            <option value="_blank" ' . (isset($value['target']) && $value['target'] == '_blank' ? 'selected="selected"' : '') . '>_blank</option>
                            <option value="_parent" ' . (isset($value['target']) && $value['target'] == '_parent' ? 'selected="selected"' : '') . '>_parent</option>
                            <option value="_top" ' . (isset($value['target']) && $value['target'] == '_top' ? 'selected="selected"' : '') . '>_top</option>
                        </select>
                        {ICON OPTIONS}
                        ' . (($options['note1']) ? '<span class="important-note">' . $options['note1'] . '</span>' : '') . '
                        <span class="error"></span>
